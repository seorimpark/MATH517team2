...
The fourth dataset is about the new cases and deaths per months. This dataset was extracted by the second dataset, by subtracting the total cases and deaths for each last days of consecutive months. This dataset was made to be used in parallel with the first dataset, and to compute the mortality rate. 
...
Below is the plot of two example states that are not neighbouring from each other, New York and California: it is the percentage of new COVID--19 cases over the total state population in New York and California as a function of time. 
...
In conclusion, the location of the State indeed has an impact on the number of cases and deaths from COVID--19. We could observe that when two states have a more similar trend when they are neighbours than when they are not. The reason behind this phenomenon would be because people would often move from one state to another closer state. For instance, we could expect that a worker living in Nevada could go to California for work, and the disease might have spread along this movement of population. It is evident that if a case outbreak happens, the disease would spread along this flow of population migrating around. 

However, there can always be an exception. On one hand, there could be as many people moving in big cities far away than the ones moving to nearby cities, and in that case the trend might be similar even though the states are not neighbouring to each other. On the other hand, we expect that this movement is not frequent with the government measures. 
...
-correlation between neighbouring states in both incidence and deaths
