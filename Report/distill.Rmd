...
This project aims to understand the spread of COVID–19 and the reasons why the pandemic has emerged as a global matter of concern in a couple of months. At the time of the pandemic outbreak, a lack of information implied awareness in the general public. By focusing on United States, we want to show the gravity of COVID–19 and analyze different periods and turning points in the pandemic management in the US. To do so, we will focus our interest on the virus transmission, number of patients and deaths during the beginning of the pandemic and until the end of the year 2021 and study its evolution with time. We will compare the situation in the different states of the US and try to find out if the evolution of the health situation in a state is predictable.
One of the limitation of this study is that at the beginning of the pandemic there was not valid test for covid and then tests for COVID--19 were not as widely used by health professionals as now.
...
2. Next, we investigate how COVID--19 deaths are distributed across some notable risk factors. In particular, we ask ourselves : How are deaths distributed across age groups ? How are deaths distributed across the recorded location of death (meaning type of institution or private residence) ?
...
`install.packages("devtools")` and `devtools::install_github("rstudio/distill")`.
...
In this section, we will have a closer look to the risk factors of death from COVID--19. More particularly, we will first consider age and second the place where death occurred.  
...
Here we can clearly see that most people were inpatients in a healthcare setting before dying. This includes ICUs and reflects our expectations. Then, people in nursing homes died also a lot as expected (older people being at risk), but much less than the previous category. The same barplot is also available by month in an interactive web app. You can access it by clicking [here](https://fahimbeck.shinyapps.io/barplot_app2/). By changing months, we are almost all the time able to observe a similar tendency : highest category being healthcare settings, and second being nursing homes. Waves can also be identified. 

## Mortality rate through time 

First we are going to plot the mortality rate and the incidence rate for the US through time to analyse their evolution. Secondly we are going to plot the proportion of deaths of COVID--19 called the case fatality ratio. This is important to measure the severity of COVID--19.
...
Now, let's compare the mortality rate to the incidence rate. The incidence rate represents the number of persons affected by COVID--19. 
...
We can see that when we plot both, the mortality rate is very small compared to the incidence rate which is normal. This shows that COVID--19 has spread quickly during these two years having a lot more cases than deaths. 
Now, we are going to compute the proportion of deaths in the country. The problem is that we don't know when a person who died at a certain month had COVID--19. There are no links between the data. So we are going to take an approximation of the proportion of deaths per month. 
...
ggplot(df_us, aes(df_us$end_date)) + 
  ...
  ggtitle("Figure (10): Evolution of the proportion of deaths ")+
  xlab("Time (in months)") + ylab("Case fatality ratio")
```


Finally, we can see that at the beginning of the spread, COVID--19 was a very mortal diseases. The peak reaches 27% : this mean that at a given time (February) more than one in four people dies of COVID--19. This information is false, in fact, the high peak can be explained by the fact that at the beginning of the pandemic, there weren't a lot of people tested. Therefore the majority of people who got tested were very sick people and so it explains why there was a very big percentage of death. After this peak, we can see that the proportion of deaths is more or less constant as we expected. This shows that the peak present in the mortality rate is explained by the high number of confirmed cases. 

## Impact of geolocation on cases and deaths from COVID--19

### Correlations depending of neighbourship

In this section, we want to validate the hypothesis that the location of the states have an impact on the number of cases and deaths from COVID--19. In order to prove it, we looked into the new number of cases and deaths from COVID--19 per day in each state, and compared between them to see if neighbouring states had a more similar trend. 

Below are the plots of two example states that are not neighbouring from each other, New York and California: the percentages of new COVID--19 cases over the total state population in New York and California are plotted as a function of time. 
...
Below are the plots of two example states that are neighbouring from each other, Nevada and California. We can observe that compared to the previous graph, the plots seem to look more alike. 
...
To check on different pairs of states, an interactive plot was made [(Link)](https://myplotseo.herokuapp.com). We can choose the pair with the two dropdowns at the top, and the states are named with its acronyms. Also, there are two modes, Cases and Deaths at the top left, which shows the plot of new cases and new deaths from COVID--19 respectively. Once the pair is chosen, the title will reveal if the two states are actually neighbours or not. 

To verify the hypothesis, we computed the Pearson correlation coefficient for each pair of States. By taking the mean of the coefficients for pairs that are neighbours and the ones for the pairs that are not neighbours, we could see a big difference (0.7575626038139122 v.s. 0.5900159155676525) We can therefore say that the trends of new COVID--19 cases are similar if the states are neighbouring from each other. We could draw a similar conclusion for the number of deaths from COVID--19, although it is not as apparent as the number of cases. (0.4804464631622195 v.s. 0.34860163565334673)

Below are the box plots of Pearson correlation coefficients between the numbers of cases from two States when they are neighbours and when they are not.

![Box Plot cases](boxplot1.png){width=70%}

Figure(13)


We can indeed observe that the Pearson correlation coefficient is significantly higher when the two States are neighbouring than when they are not. 

Below are the box plots of Pearson correlation coefficients between the numbers of deaths from two States when they are neighbours and when they are not.

![Box Plot deaths](boxplot2.png){width=70%}

Figure(14)

The contrast for the number of new deaths is not a apparent as the number of new cases, but we can still retrieve the result we expected.

In conclusion, the location of the State indeed has an impact on the number of cases and deaths from COVID--19. We could observe that two states have a more similar trend when they are neighbours than when they are not. There were several waves of infection in different places, and we could observe a lagging peak in cases of farther states. The reason behind this phenomenon is because people would often move from one state to another closer state. For instance, we could expect that some people living in Nevada could go to California for work, and the disease might have spread along this movement of population. However, there can also be as many people moving to big farther cities as the ones moving to nearby cities, and in that case the trend can be similar even though the states are not neighbouring to each other. For instance, New York and California both have a peak in January, although they are situated on the both extremities of the country. This phenomenon might have occurred because those two states include the two biggest city in the U.S. -- Los Angeles and New York City -- and the migration of population between the two cities might have been frequent at that moment. 


### Predicting deaths from k-th degree neighbors
In this section, we propose and implement a linear model to predict the number of monthly COVID--19 deaths in a given state as a function of the number of deaths during the same month in a k-th degree neighbour of that state. Two states X and Y are k-th degree neighbours if at least k interstate borders must be crossed to go from one state to the other. 

$$
E[N_X\mid N_Y] = b_0^{(k_{XY})}+ b_1^{(k_{XY})} N_Y
$$
For example, the above model can be used to predict the number of COVID--19 deaths in Massachusetts during a month given the number of deaths in Vermont during the same month, where Vermont and Massachusetts share a border and are thus 1st degree neighbours ($k=1$). The panel of figures below contains scatterplots of $N_X$ against $N_Y$ for any month and any two states that are neighbours of degree $d$. In order to find the degree of neighbourship between two states, we have treated the states as nodes on a graph, where edges correspond to borders between states. We constructed an adjacency matrix between states sharing a border, and subsequently we used the Floyd–Warshall algorithm to find the smallest number of interstate borders connecting any two states. In each panel, we have indicated the parameters $b_0$, $b_1$ and the correlation coefficient $\rho$ between $N_X$ and $N_Y$. 
...
As expected, the predictive power of a neighbour decreases with increasing degree of the neighbour. We see that the variation in the number of deaths is large, and thus it is (unsurprisingly) hard to make accurate predictions of the number of deaths. The speed of attenuation of response $b_1$ with $d$ nonetheless sheds light on the spatial scale of the correlations between the number of deaths.


## Discussion and conclusion 

In this project, we have aimed to descriptively characterize the extent of COVID-19 deaths in the US during the global pandemic. We began by visualizing a dynamic heatmap in which we noticed a wave of increasing deaths from the south to the north of the US between July (307 deaths/million in Arizona,~253 deaths/million in Texas, ~245 deaths/million in Florida VS North Dakota  ~40 deaths/million,Wisconsin  28deaths/million , Maine  18deaths/million) and December 2020(441 deaths/million in Arizona,~275 deaths/million in Texas, ~155 deaths/million in Florida VS North Dakota  ~461 deaths/million,Wisconsin 306 deaths/million , Maine  153 deaths/million)  : US map plots(develop more). As expected, we found that older individuals are overrepresented in the mortality statistics. Most people dying from Covid are older than 50-60 years. We also noticed periods of time with a substantial increase in mortality amongst infected individuals, which might be explained by a of shortage of available intensive care facilities in hospitals: [Many Hospitals With No Beds Left Are Forced To Send COVID Patients To Cities Far Away](https://www.npr.org/2021/08/19/1029378744/hospital-beds-shortage-covid-coronavirus-states?t=1635160291866) ). Next, we observed that the correlation in number of deaths and cases of infection was greater for neighboring states than for states that are further away from each other. Based on this observation, we constructed a linear model to predict the number of deaths in a given state from the number of deaths in a k-th degree neighbor. As expected, the predictive power of the model decreased with the increasing degree of separation of the states. The predictions were most accurate in regimes with a low number of deaths.  Overall, the findings cast light on the magnitude of deaths and distribution of deaths across key factors during the COVID-19 pandemic in the US.

## Future improvements 

As a further improvement and extension of the research, we suggest incorporating a forecasting model and visually displaying it in the same manner.
We can also extend the following study and add more parameters such as health restrictions imposed by the government and their impact on the spread and deaths.
We can further add the availability of hospitals as a parameter. 
With interesting and conclusive results, we can broaden our study to other countries or even other epidemics.
Furthermore, another dimension to consider would be the vaccination: does it have a positive or negative effect on mortality?
Finally, comorbidity can be considered. 
